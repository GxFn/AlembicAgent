...
4. 如果有当前计划、任务包或 VAD heartbeat，只按 `../codex-control-workspace/.workspace-active/workspace/current` 中明确分配给 `AlembicAgent` 的内容执行。
...
- VAD 模式下只允许 claim / finish `AlembicAgent` 对应任务；`claim --json` 没有返回本窗口任务时必须停止。
