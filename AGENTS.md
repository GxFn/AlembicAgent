...
- 新建迁移说明、协作文档或跨仓库设计记录时，统一保存到 workspace 根目录的 `docs/AlembicAgent/`，不要散落到各子仓库。
- 需要引用 AlembicCore 迁移计划、公开 API 边界或外层接入任务时，统一查看 workspace 根目录的 `docs/AlembicCore/`；Agent 自身计划仍放 `docs/AlembicAgent/`。
...
- workspace 级迁移和协作文档保存在 workspace 根目录的 `docs/AlembicAgent/`。
...
- 可以使用中文注释解释 Agent 边界、工具权限、安全策略、复杂状态机或宿主兼容原因；不要给自解释代码堆注释。
