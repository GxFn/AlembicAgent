...
- 只允许 claim / finish `AlembicAgent` 目标任务；不得代领、处理、验证或总结其它窗口任务。
...
- 宿主接入 adapter contract，例如 CLI agent、local daemon agent 或后续宿主需要注入的 Agent/tool/context 边界；Codex MCP server、marketplace、channel 和 host-agent route 不在本仓库实现。
...
- Codex MCP server、Codex marketplace/channel、Codex Skill 文案和插件发布链路属于插件交付仓库。
