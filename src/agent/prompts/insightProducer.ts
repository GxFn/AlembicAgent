...
- 涉及「依赖 / 调用 / 关系 / 上游 / 下游」等关系声明时：若下方提供了「可复制 graphRefs」，逐字复制相关条目进 reasoning.graphRefs；没有提供时改述为边界 / 导入方向规则并避开这些关系词。绝不自行编造 graphRefs（否则 GRAPH_REF_INVALID）。
...
export function buildProducerPromptV2(
  ...
  if (artifact.graphEvidence && artifact.graphEvidence.length > 0) {
    ...
    graphLines.push(
      '候选含「依赖 / 调用链 / 上游 / 下游 / 关系」等声明时，从下面逐字复制相关条目放进 reasoning.graphRefs（数组）；与声明无关的条目不要复制。'
    );
