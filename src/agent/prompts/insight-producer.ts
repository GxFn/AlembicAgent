...
3. 标注相关文件的 canonical repo-relative sourceRefs + 行号（必须来自提供的 sourceRef index / Analyst 已验证路径，如 Packages/AOXNetworkKit/Sources/.../NetworkClient.swift:42）
...
- reasoning.sources 必须是非空数组，填写 sourceRef index 中的 canonical repo-relative path，如 ["Packages/AOXNetworkKit/Sources/AOXNetworkKit/Client/NetworkClient.swift"]（禁止只写文件名）
- sourceRefs / reasoning.sources 是严格合同：必须复用 sourceRef index / Analyst 已读取代码里的真实相对路径；禁止凭空改扩展名、只写 basename、写模块别名、类名或猜测不存在路径；无法确认 canonical path 时跳过该候选，不要提交
- content.markdown 和其它 source labels 中的来源标注必须使用 canonicalSourceRefIndex 里的完整相对路径+行号: (来源: Full/Path/FileName.ext:行号)；basename-only / entity-name-only 来源会被 knowledge.submit strict gate repair 或 reject
...
- 先确认 sourceRefs 来自 canonical index，再提交候选；sourceRef 不确定时宁可少提交，不要编造路径`;
...
export function buildProducerPromptV2(
  ...
  const groundingSection = buildSourceRefGroundingSection(artifact);
  if (groundingSection) {
...
export function buildSourceRefGroundingSection(artifact: AnalysisArtifactLike): string | null {
  const trustedRefs = new Set<string>();
  for (const file of artifact.referencedFiles || []) {
    if (file?.trim()) {
      trustedRefs.add(file.trim());
    }
  }
  if (artifact.evidenceMap instanceof Map) {
    for (const file of artifact.evidenceMap.keys()) {
      if (file?.trim()) {
        trustedRefs.add(file.trim());
      }
    }
  ...
  if (trustedRefs.size === 0) {
    return null;
  }

  const sourceRefIndex = [...trustedRefs].slice(0, 24).map((sourcePath, index) => ({
    basename: sourcePath.split('/').pop() || sourcePath,
    id: `file:${String(index + 1).padStart(3, '0')}`,
    path: sourcePath,
  }));
  const uniqueBasenameExample = sourceRefIndex.find((entry) => entry.path.includes('/'));
  const markdownExample = sourceRefIndex.find((entry) => entry.path.endsWith('.md'));
  const exampleLines = [
    markdownExample
      ? `- wrong: ${markdownExample.path.replace(/\.md$/, '.m')}；right: ${markdownExample.path}`
      : '- wrong: README.m；right: README.md（必须以实际 index 为准）',
    uniqueBasenameExample
      ? `- wrong: ${uniqueBasenameExample.basename}；right: ${uniqueBasenameExample.path}`
      : null,
  ].filter(Boolean);

  return [
    '## SourceRef strict contract',
    'sourceRefs / reasoning.sources / content.markdown 来源标注 / source labels 都不是自由文本，必须来自下方 canonicalSourceRefIndex 或 Analyst 已验证的 repo-relative path。',
    'knowledge.submit 会在 bootstrap Producer 阶段执行 strict sourceRef gate：可唯一修复的 basename / extension 会被 repair 并回写正文；ambiguous / entity-only / outside-root / package-path mismatch 会被 reject。',
    '如果无法定位 canonical path，跳过该候选或在正文中说明不确定原因；不要提交猜测路径。',
    'canonicalSourceRefIndex:',
    ...sourceRefIndex.map((entry) => `- ${entry.id}: ${entry.path} (basename: ${entry.basename})`),
