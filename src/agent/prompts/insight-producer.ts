...
2. 优先使用结构化发现中的 evidence 和 Analyst evidence refs；只有缺少精确代码片段时，才用 code({ action: "read", params: { path: "Full/Path/To/File.swift", startLine: N, endLine: M } }) 小范围补读
...
- 如果 Analyst evidence refs 已提供文件、行号和证据摘要，不要为了相同证据重复 code.read；只有缺少 content.markdown 所需的短代码片段时才补读
...
- 每次 knowledge.submit 前先自检 params.description 非空；缺少 description 会被工具拒绝并浪费 Producer 轮次
...
- 不新增搜索探索；优先使用 Analyst evidence refs，只有缺少精确短代码片段时才小范围 code.read
 ...
 * Producer 如果确实需要 content.markdown 的精确短片段，可以按 refs 小范围 code.read；
 * 这样避免 live history 同时保留 direct code replay、额外 code.read 和 submit payload。
 */
export function buildCodeContextSection(
  ...
  const parts = [
    ...
    '这些 refs 已由 Analyst 验证；不要为相同证据重复 code.read。只有 content.markdown 缺少精确短代码片段时，才按行号小范围补读。',
  ];
  let totalChars = 0;
  const BUDGET = 1600;
  ...
  for (const entry of sortedEntries) {
    ...
    const ranges = entry.codeSnippets
      .slice(0, 3)
      .map((snippet) => `L${snippet.startLine}-${snippet.endLine}`)
      .join(', ');
    const summary = entry.summary ? ` — ${limitText(entry.summary, 180)}` : '';
    const line = `- ${entry.filePath}${entry.role ? ` (${entry.role})` : ''}${ranges ? ` [${ranges}]` : ''}${summary}`;
    parts.push(line);
