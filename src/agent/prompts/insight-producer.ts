...
- sourceRefs / reasoning.sources 必须优先复用“分析中引用的关键文件”和“Analyst 已读取的代码”里的真实相对路径；禁止凭空改扩展名、只写 basename、写模块别名或猜测不存在路径
...
export function buildSourceRefGroundingSection(artifact: AnalysisArtifactLike): string | null {
  ...
  const examples = [...trustedRefs].slice(0, 12);
  return [
    '## SourceRef grounding policy',
    '提交 knowledge.submit 时，sourceRefs / reasoning.sources 必须锚定到 Analyst 已验证的真实相对路径。',
    `可优先使用的已验证路径: ${examples.join(', ')}`,
    '- 如果候选来自上述文件，直接复用完整相对路径，可保留行号，例如 path/to/File.ts:42。',
    '- 不要只写文件名、模块名、目录别名或把源码扩展名猜成另一种扩展名。',
    '- 如果无法确认真实路径，保留正文证据说明，但不要在 sourceRefs 中编造路径；后续 N11 scorecard 会保留并度量 invalid refs。',
  ].join('\n');
}
