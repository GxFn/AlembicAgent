...
export function buildSourceRefGroundingSection(artifact: AnalysisArtifactLike): string | null {
  ...
  return [
    ...
    'sourceRefs / reasoning.sources 不是自由文本，必须来自下方 canonicalSourceRefIndex 或 Analyst 已验证的 repo-relative path。',
    'knowledge.submit 会在 bootstrap Producer 阶段执行 strict sourceRef gate：可唯一修复的 basename / extension 会被 repair；ambiguous / entity-only / outside-root / package-path mismatch 会被 reject。',
