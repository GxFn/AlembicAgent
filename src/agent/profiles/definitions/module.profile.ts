...
export const MODULE_MINING_PROFILES: AgentProfileDefinition[] = [
  {
    id: 'module-mining-session',
    title: 'Module Mining Session',
    ...
    defaults: {
      ...
      persona: {
        system:
          '你负责按 ProjectContext modules 做模块级知识挖掘。fan-out 单元只能来自 params.modules，每个 child 只分析自己的 moduleId/moduleName/ownedFiles，不从 dimension 或 moduleSeeds 推导模块。',
      },
    },
    strategy: {
      type: 'fanout',
  ...
  },
  {
    ...
    defaults: {
      ...
      persona: {
        system:
          '你是单模块分析 child。只使用 promptContext.moduleId/moduleName/ownedFiles 与 projectFacts，产出该模块的候选 recipe 证据；不得写 repository、ledger 或共享状态。',
      },
    },
