...
describe('冷启动候选拒绝修复：copy-ready 证据 → cold-start 门禁可过 (options 2+3)', () => {
  ...
  it('超预算 bounded 代码截断仍是源码逐字前缀，且 fixture 不使用 whole-file 范围', () => {
    const fullContent = fileLines('alpha', 2, 20); // interior bounded range，~500 字符 > 220 上限
    const section = buildCodeContextSection(
      new Map([
        [
          'src/alpha.ts',
          {
            ...
            summary: 'Alpha 全文件',
            codeSnippets: [{ startLine: 2, endLine: 20, content: fullContent }],
          },
        ],
      ])
    );
    ...
    const marker = '可复制 coreCode(来源 src/alpha.ts:2-20): ';
