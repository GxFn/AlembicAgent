...
describe('冷启动候选拒绝修复：copy-ready 证据 → cold-start 门禁可过 (options 2+3)', () => {
  ...
  it('F4c: coreCode 为真而 markdown fenced 自创时，规范化变体（fenced←coreCode）过完整门禁', async () => {
    // 直接经 handleSubmit 域外不可行（需 gateway 全套），此处钉 handler 内规范化所依赖的
    // 不变量：真实 coreCode + 自创 fenced 的候选被拒；把 fenced 替换为 coreCode 后零违规。
    // 与 beforeAll 写入的 src/alpha.ts 前三行逐字一致。
    const realCode = Array.from({ length: 3 }, (_, i) => `export const alpha${i} = ${i};`).join(
      '\n'
    );
    const fabricated = cleanRecipe({
      coreCode: realCode,
      content: {
        markdown: [
          '## Alpha 模块单向 import 约定',
          ...
          '```ts',
          'const fabricatedIllustration = rewriteForReadability();',
          '```',
          '✅ Keep alpha module imports flowing one direction only.',
          '❌ Do not import beta or gamma back into alpha.',
        ].join('\n'),
    ...
    const rejected = runInProcessRecipeAuthoringGate(fabricated, {
      projectRoot,
      dimensionId: 'architecture',
    });
    expect(codesOf(rejected)).toContain('SNIPPET_MISMATCH');

    const normalized = {
      ...fabricated,
      content: {
        ...(fabricated.content as Record<string, unknown>),
        markdown: (fabricated.content as { markdown: string }).markdown.replace(
          /```ts\n[\s\S]*?```/,
          `\`\`\`ts\n${realCode}\n\`\`\``
        ),
      },
    };
    const violations = runInProcessRecipeAuthoringGate(normalized, {
      projectRoot,
      dimensionId: 'architecture',
    });
    expect(violations).toEqual([]);
